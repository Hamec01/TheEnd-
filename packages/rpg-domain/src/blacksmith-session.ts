...
export function createBlacksmithSession(seed: BlacksmithSessionSeed, bonuses: BlacksmithSessionBonuses = {}): BlacksmithSessionState {
...
export function applyBlacksmithAction(
  state: BlacksmithSessionState,
  action: 'prepare_blank' | 'add_heat' | 'stabilize_heat' | 'light_strike' | 'medium_strike' | 'heavy_strike' | 'quench_water' | 'quench_oil' | 'finish_polish',
  bonuses: BlacksmithSessionBonuses = {},
): BlacksmithSessionActionResult {
  if (state.completed) {
  ...
  if (action === 'finish_polish') {
    ...
    if (alreadyRefined) {
      next.defectScore = clamp(
        next.defectScore + Math.max(2, 6 - Math.round((bonuses.defectChanceReduction ?? 0) / 6)),
        0,
        100,
      );
    } else {
...
export function finalizeBlacksmithScore(state: BlacksmithSessionState, bonuses: BlacksmithSessionBonuses = {}): number {
