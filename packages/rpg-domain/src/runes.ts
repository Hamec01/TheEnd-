...
export interface RuneCost {
  ...
  stamina?: number;
  mp?: number;
  memoryLossRisk?: number;
  soulDamageRisk?: number;
  selfDebuffId?: string;
  riskChance?: number;
}
...
export interface RuneDefinition {
  ...
  cost: RuneCost;
  effects: RuneEffect[];
  requiresShamanKnowledge?: boolean;
  canBeCarvedByDwarf?: boolean;
  forbidden?: boolean;
  unstableWithoutBindingRune?: boolean;
}
...
export interface RuneComplex {
  ...
  riskChance: number;
  totalCost: RuneCost;
  combinedEffects: RuneEffect[];
  isUnstable: boolean;
}

export interface RuneUsageResult {
  success: boolean;
  appliedEffects: RuneEffect[];
  paidCost: RuneCost;
  backlash?: RuneBacklash;
}

export interface RuneBacklash {
  type:
    | 'hp_loss'
    | 'stamina_loss'
    | 'silence'
...
export interface CharacterLike {
  ...
  currentMp: number;
  willpower: number;
  knowsRuneCraft?: boolean;
  canCarveRunes?: boolean;
  canUseRuneItems?: boolean;
  hasForbiddenRunePermission?: boolean;
}

export const BINDING_RUNE_IDS = ['hald', 'elgar', 'kragnor', 'nurrak', 'feldr', 'orkann', 'harran'] as const;

function clamp(value: number, min: number, max: number): number {
  return Math.max(min, Math.min(max, value));
}

function mergeCost(a: RuneCost, b: RuneCost): RuneCost {
  return {
    hp: (a.hp ?? 0) + (b.hp ?? 0),
...
function createRune(definition: RuneDefinition): RuneDefinition {
  return definition;
}

const BINDING_RUNES: RuneDefinition[] = [
  createRune({
    id: 'hald',
    name: 'Хальд',
    originalName: 'Hald',
...
const COMBAT_RUNES: RuneDefinition[] = [
  createRune({
    id: 'uragg',
    name: 'Урагг',
    originalName: 'Uragg',
...
const PROTECTIVE_RUNES: RuneDefinition[] = [
  createRune({
    id: 'farn',
    name: 'Фарн',
    originalName: 'Farn',
...
const RITUAL_AND_FORBIDDEN_RUNES: RuneDefinition[] = [
  createRune({
    id: 'mordr',
    name: 'Мордр',
    originalName: 'Mordr',
...
const UTILITY_RUNES: RuneDefinition[] = [
  createRune({
    id: 'rung',
    name: 'Рунг',
    originalName: 'Rung',
...
export const RUNE_DEFINITIONS: Record<string, RuneDefinition> = [
  ...BINDING_RUNES,
  ...COMBAT_RUNES,
  ...PROTECTIVE_RUNES,
  ...RITUAL_AND_FORBIDDEN_RUNES,
  ...UTILITY_RUNES,
].reduce<Record<string, RuneDefinition>>((acc, rune) => {
  acc[rune.id] = rune;
  return acc;
}, {});

function hasRunePrice(cost: RuneCost): boolean {
  return (cost.hp ?? 0) > 0
    || (cost.stamina ?? 0) > 0
    || (cost.mp ?? 0) > 0
    || (cost.riskChance ?? 0) > 0
...
function isPowerfulRune(rune: RuneDefinition): boolean {
...
export function ensureRuneDesignSafety(definitions: Record<string, RuneDefinition> = RUNE_DEFINITIONS): void {
...
function isBindingRune(runeId: string): boolean {
  return BINDING_RUNE_IDS.includes(runeId as (typeof BINDING_RUNE_IDS)[number]);
}

function resolveRunes(runeDefinitions: RuneDefinition[], runeIds: string[]): RuneDefinition[] {
  const byId = new Map(runeDefinitions.map((rune) => [rune.id, rune]));
  return runeIds.map((id) => byId.get(id)).filter((rune): rune is RuneDefinition => Boolean(rune));
}

function getPowerModifier(runes: RuneDefinition[]): number {
...
export function calculateRuneComplex(
  runeDefinitions: RuneDefinition[],
  runeIds: string[],
): RuneComplex {
  ...
  const totalCost = runes.reduce<RuneCost>((cost, rune) => mergeCost(cost, rune.cost), {});
  ...
  const stability = clamp(
    Math.round(100 - risk * 100 + bindingCount * 8 + stabilizePower * 0.4 - amplifyCount * 6 - Math.max(0, combatOrForbiddenCount - 3) * 5),
    0,
    100,
  );
...
export function getDefaultRunePermissionsForRace(race: Race | string): Pick<CharacterLike, 'knowsRuneCraft' | 'canCarveRunes' | 'canUseRuneItems'> {
...
export function canCharacterCreateRuneComplex(character: CharacterLike): boolean {
  return character.knowsRuneCraft === true;
}

export function canCharacterUseRune(
...
export function calculateRuneBacklashChance(complex: RuneComplex, userWillpower: number): number {
...
export function rollRuneBacklash(
  complex: RuneComplex,
  userWillpower: number,
): RuneBacklash | null {
