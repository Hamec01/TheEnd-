...
type ItemEffectType = 'heal_hp' | 'restore_mana' | 'restore_stamina' | 'damage_target' | 'apply_status' | 'remove_status';
...
type ResolvedItemEventType = 'item_used' | 'heal' | 'resource_restore' | 'damage' | 'status_applied' | 'status_removed' | 'item_effect_skipped';

export interface ResolvedItemEvent {
  type: ResolvedItemEventType;
  actorId: string;
  targetId?: string;
  itemId?: string;
...
function normalizeTarget(value: unknown): NormalizedItemEffect['target'] {
...
function normalizeItemEffects(item: ItemDefinition, itemData?: unknown): NormalizedItemEffect[] {
  ...
  const out: NormalizedItemEffect[] = [];
...
function resolveTargets(params: {
  ...
  commandTarget: CombatCommand['target'];
}): ArenaCombatEntity[] {
...
export function resolveItemUse(params: {
  ...
  const events: ResolvedItemEvent[] = [];
  const errors: CombatPlanErrorCode[] = [];
  const warnings: CombatPlanWarningCode[] = [];
  ...
  const item = params.item ?? getItemById(itemId);
