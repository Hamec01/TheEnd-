...
function countContent(db: ContentDatabase): Record<string, number> {
  return {
    ...
    diplomaticActors: (db.diplomaticActors ?? []).length,
    globalRelations: (db.globalRelations ?? []).length,
    ...
    locations: Array.isArray(input.locations)
      ? input.locations
        .map((location) => ({
          ...
          workshopIds: Array.isArray(location.workshopIds) ? location.workshopIds.map((entry) => String(entry).trim()).filter(Boolean) : [],
          services: Array.isArray(location.services) ? location.services.map((entry) => String(entry).trim()).filter(Boolean) : [],
...
export class ContentService implements OnModuleInit, OnModuleDestroy {
  ...
  private normalizeDatabase(raw: Partial<ContentDatabase>): ContentDatabase {
    return {
      ...
      diplomaticActors: clone(sanitizeIdObjectArray<DiplomaticActorDefinition>(raw.diplomaticActors)).filter((actor) => Boolean(actor.id)),
      globalRelations: clone(sanitizeIdObjectArray<GlobalRelation>(raw.globalRelations)).filter((relation) => Boolean(relation.id)),
