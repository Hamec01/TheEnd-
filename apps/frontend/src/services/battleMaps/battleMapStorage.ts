...
export function normalizeBattleMap(map: Partial<BattleMapDefinition>): BattleMapDefinition {
  ...
  return {
    ...
    musicAssetId: typeof map.musicAssetId === 'string' && map.musicAssetId.trim().length > 0 ? map.musicAssetId.trim() : undefined,
    musicUrl: typeof map.musicUrl === 'string' && map.musicUrl.trim().length > 0 ? map.musicUrl.trim() : undefined,
    ambientAssetId: typeof map.ambientAssetId === 'string' && map.ambientAssetId.trim().length > 0 ? map.ambientAssetId.trim() : undefined,
    ambientUrl: typeof map.ambientUrl === 'string' && map.ambientUrl.trim().length > 0 ? map.ambientUrl.trim() : undefined,
    cellSizePx,
    gridOffsetX,
    gridOffsetY,
    logicalColumns,
