...
export function CharacterSkillsPage(props: CharacterSkillsPageProps) {
  const { learnedSkills, availableSkills, loadout, onLearnSkill, onSaveLoadout, onStatus } = props;
  ...
  const learnableSkills = useMemo(
    () => availableSkills.filter((skill) => skill.isPublished && !skill.isHidden && !learnedSkillIds.has(skill.id)),
  ...
  return (
    <div className="skills-inventory-shell">
      <div className="skills-inventory-top-row">
      ...
      <div className="skills-inventory-bottom-row">
        <section className="inner-card skills-training-section">
          <div className="skills-section-head">
            <div>
              <h3 style={{ margin: 0 }}>Обучение</h3>
              <span className="muted">Показываются опубликованные навыки, которых у персонажа ещё нет.</span>
            </div>
          </div>
        ...
        <section className="inner-card skills-loadout-section">
          <div className="skills-section-head">
            <div>
