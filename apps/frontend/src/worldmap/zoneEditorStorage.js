...
const REGION_TYPES = ['walkable', 'blocked', 'water', 'road', 'danger', 'trigger'];
...
export function normalizeZone(input) {
    ...
    if ((shape !== 'circle' && shape !== 'polygon' && shape !== 'rect') || !ZONE_TYPES.includes(type)) {
        return null;
    }
    const normalized = {
...
export function validateZonesJson(text) {
...
export function validateEditorDataJson(text) {
    const errors = [];
    let parsed;
    try {
    ...
    const rawZones = Array.isArray(parsed)
        ? parsed
        : parsed && typeof parsed === 'object' && Array.isArray(parsed.zones)
            ? parsed.zones
            : null;
    const rawRegions = parsed && typeof parsed === 'object' && Array.isArray(parsed.regions)
        ? parsed.regions
        : [];
    if (!rawZones) {
...
export function exportZonesJson(zones) {
    return JSON.stringify(zones, null, 2);
}
export function exportEditorDataJson(zones, regions) {
    return JSON.stringify({ zones, regions }, null, 2);
}
export function loadEditorDataFromStorage(initialZones) {
...
export function saveEditorDataToStorage(zones, regions) {
    window.localStorage.setItem(DEV_ZONE_STORAGE_KEY, exportEditorDataJson(zones, regions));
}
export function loadZonesFromStorage(initialZones) {
    return loadEditorDataFromStorage(initialZones).zones;
}
export function saveZonesToStorage(zones) {
...
export function clearEditorSettingsStorage() {
