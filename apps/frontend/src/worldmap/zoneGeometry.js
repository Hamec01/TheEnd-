...
export function screenToMapNormalized(screenX, screenY, viewport) {
...
export function mapNormalizedToScreen(x, y, viewport) {
    return [
...
export function hitTestZones(zones, point) {
...
export function hitTestHandle(zone, point, viewport) {
    const tolerance = 10;
    if (zone.shape === 'circle') {
...
export function moveZone(zone, deltaX, deltaY) {
    if (zone.shape === 'circle') {
        return {
            ...zone,
    ...
    return {
        ...zone,
        points: (zone.points ?? []).map(([x, y]) => [clamp(x + deltaX, 0, 1), clamp(y + deltaY, 0, 1)]),
        updatedAt: Date.now(),
    };
}
export function resizeCircle(zone, point) {
...
export function movePolygonPoint(zone, pointIndex, toPoint) {
    if (zone.shape === 'circle') {
        return zone;
    }
    return {
        ...zone,
        points: (zone.points ?? []).map((point, index) => (index === pointIndex ? [clamp(toPoint[0], 0, 1), clamp(toPoint[1], 0, 1)] : point)),
        updatedAt: Date.now(),
    };
}
