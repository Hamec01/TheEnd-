...
const BELLOWS_HIT = { x: 300, y: 410, w: 150, h: 150 };
const ANVIL_HIT = { x: 500, y: 380, w: 280, h: 150 };
const STRIKE_ZONE_X = { left: 540, center: 640, right: 740 };
const STAGE1_MAX_MS = 12000;
...
function startForgeGame(ctx) {
    const hudHeat = requireElement('hud-heat');
    const hudShape = requireElement('hud-shape');
    const hudQuality = requireElement('hud-quality');
    const hudDefects = requireElement('hud-defects');
    const hudProgress = requireElement('hud-progress');
    const eventOverlay = requireElement('event-overlay');
    const quenchOverlay = requireElement('quench-overlay');
    const finishOverlay = requireElement('finish-overlay');
    const resultOverlay = requireElement('result-overlay');
    ...
    function spawnSteam(x, y, count = 18) {
        for (let i = 0; i < count; i += 1) {
            particles.push({
    ...
    function getQualityTier(score) {
        if (state.defects >= 70)
            return { tier: 'Брак (Металлолом)', color: '#ff4444' };
        if (state.defects >= 45)
            return { tier: 'Деформированная пластина', color: '#ffaa44' };
        if (score < 40)
            return { tier: 'Хлам (Грубая работа)', color: '#9d9d9d' };
        if (score < 65)
            return { tier: 'Обычное качество', color: '#ffffff' };
        if (score < 80)
            return { tier: 'Редкое изделие', color: '#0070dd' };
        if (score < 92)
            return { tier: 'Мастерская ковка', color: '#a335ee' };
    ...
    function setStage(newStage) {
        ...
        if (newStage === 'BELLOWS') {
            ...
            finishPointsLeft = 3;
            requireElement('finish-points').innerText = String(finishPointsLeft);
    ...
    function triggerRandomEvent() {
        ...
        requireElement('event-title').innerText = randomEvent.title;
        requireElement('event-desc').innerText = randomEvent.description;
        eventOverlay.classList.add('active');
        requireElement('btn-fix').onclick = () => randomEvent.onFix();
        requireElement('btn-ignore').onclick = () => randomEvent.onIgnore();
    }
    requireElement('btn-quench-water').onclick = () => {
        selectedFluid = 'water';
        quenchOverlay.classList.remove('active');
    };
    requireElement('btn-quench-oil').onclick = () => {
        selectedFluid = 'oil';
        quenchOverlay.classList.remove('active');
    };
    function updateFinishPoints() {
        requireElement('finish-points').innerText = String(finishPointsLeft);
        if (finishPointsLeft <= 0) {
            setStage('RESULTS');
        }
    }
    requireElement('btn-finish-balance').onclick = () => {
        if (finishPointsLeft <= 0)
            return;
    ...
    requireElement('btn-finish-polish').onclick = () => {
        if (finishPointsLeft <= 0)
            return;
    ...
    requireElement('btn-finish-sharpen').onclick = () => {
        if (finishPointsLeft <= 0)
            return;
    ...
    function showResults() {
        ...
        const titleEl = requireElement('result-title');
        const descEl = requireElement('result-desc');
        const gradeEl = requireElement('result-grade');
        if (state.defects >= 70) {
    ...
    requireElement('btn-restart').onclick = () => {
        state.heat = 20;
        state.shape = 0;
        state.quality = 50;
        state.defects = 0;
    ...
    function handleStrike(zone) {
        const now = performance.now();
        if (now - lastStrikeTime < STRIKE_COOLDOWN_MS)
            return;
        lastStrikeTime = now;
    ...
    canvas.addEventListener('mousedown', (event) => {
        ...
        if (state.stage === 'STRIKING' && pointInRect(clickX, clickY, ANVIL_HIT)) {
            let hitZone = 'center';
            if (clickX < STRIKE_ZONE_X.center - 50)
                hitZone = 'left';
            else if (clickX > STRIKE_ZONE_X.center + 50)
                hitZone = 'right';
    ...
    function drawSprite(name, fallback) {
